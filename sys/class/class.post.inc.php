...
class post extends db_connect
{
    ...
    public function remove($postId, $allowRestore = 0)
    {
        $result = array(
            "error" => true,
            "error_code" => ERROR_SUCCESS
        ...
        if ($postInfo['groupId'] != 0) {
            ...
            // Posts made in groups cannot be restored

            $allowRestore = 0;
        }

        if ($postInfo['fromUserId'] != $this->getRequestFrom()) {
        ...
        $stmt = $this->db->prepare("UPDATE posts SET removeAt = (:removeAt), allowRestore = (:allowRestore), restoreAt = 0, pinned = 0 WHERE id = (:postId)");
        $stmt->bindParam(":postId", $postId, PDO::PARAM_INT);
        $stmt->bindParam(":removeAt", $currentTime, PDO::PARAM_INT);
        $stmt->bindParam(":allowRestore", $allowRestore, PDO::PARAM_INT);
    ...
    public function restore($postId)
    {
        ...
        $stmt = $this->db->prepare("UPDATE posts SET removeAt = 0, restoreAt = (:restoreAt) WHERE id = (:postId)");
        $stmt->bindParam(":postId", $postId, PDO::PARAM_INT);
        $stmt->bindParam(":restoreAt", $currentTime, PDO::PARAM_INT);
    ...
    public function pin($itemId)
    {
        ...
        $stmt = $this->db->prepare("UPDATE posts SET pinned = 0 WHERE fromUserId = (:fromUserId) AND pinned = 1");
        $stmt->bindParam(":fromUserId", $this->requestFrom, PDO::PARAM_INT);
        ...
        if ($stmt->execute()) {
            ...
            if ($itemInfo['pinned'] == 0) {

                $stmt = $this->db->prepare("UPDATE posts SET pinned = 1 WHERE id = (:itemId)");
    ...
    public function quikInfo($row)
    {
        ...
        $result = array(
            ...
            "removeAgo" => $time->timeAgo($row['removeAt']),
            "restoreAt" => $row['restoreAt'],
            "allowRestore" => $row['allowRestore'],
            "ip_addr" => $row['ip_addr'],
            "rePost" => array(),
            "owner" => $profileInfo);
    ...
    public function get($profileId, $postId = 0, $accessMode = 0)
    {
        ...
        if ($postId == 0) {
            ...
            if ($accessMode == 0) {
                ...
                $stmt = $this->db->prepare("SELECT id FROM posts WHERE pinned = 1 AND accessMode = 0 AND fromUserId = (:fromUserId) AND groupId = 0 AND removeAt = 0 ORDER BY id DESC LIMIT 1");
                $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);

            } else {

                $stmt = $this->db->prepare("SELECT id FROM posts WHERE pinned = 1 AND fromUserId = (:fromUserId) AND groupId = 0 AND removeAt = 0 ORDER BY id DESC LIMIT 1");
                $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);
            }
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    array_push($posts['posts'], $itemInfo);
        ...
        if ($accessMode == 0) {
            ...
            $stmt = $this->db->prepare("SELECT id FROM posts WHERE pinned = 0 AND accessMode = 0 AND fromUserId = (:fromUserId) AND groupId = 0 AND removeAt = 0 AND id < (:postId) ORDER BY id DESC LIMIT 20");
            $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);
            $stmt->bindParam(':postId', $postId, PDO::PARAM_INT);

        } else {

            $stmt = $this->db->prepare("SELECT id FROM posts WHERE pinned = 0 AND fromUserId = (:fromUserId) AND groupId = 0 AND removeAt = 0 AND id < (:postId) ORDER BY id DESC LIMIT 20");
            $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);
            $stmt->bindParam(':postId', $postId, PDO::PARAM_INT);
        }
