...
class update extends db_connect
{
    ...
    function addColumnToPostsTable16()
    {
        $stmt = $this->db->prepare("ALTER TABLE posts ADD restoreAt INT(11) UNSIGNED DEFAULT 0 after needToModerate");
        $stmt->execute();
    }

    function addColumnToPostsTable17()
    {
        $stmt = $this->db->prepare("ALTER TABLE posts ADD allowRestore INT(6) UNSIGNED DEFAULT 0 after restoreAt");
        $stmt->execute();
    }

    // for version 5.9

    function addColumnToPostsTable18()
    {
        $stmt = $this->db->prepare("ALTER TABLE posts ADD pinned INT(10) UNSIGNED DEFAULT 0 after id");
        $stmt->execute();
    }
    ...
    public function updateUsersTable47()
    {
        $stmt = $this->db->prepare("UPDATE users SET gl_id = '' WHERE gl_id = 0");
        $stmt->execute();
    }

    function modifyUsersTable1()
    {
        $stmt = $this->db->prepare("ALTER TABLE users MODIFY COLUMN gl_id varchar(40) NOT NULL DEFAULT '' ");
        $stmt->execute();
    }

    //

    function modifySettingsTable1()
    {
        $stmt = $this->db->prepare("ALTER TABLE settings MODIFY COLUMN textValue CHAR(64) NOT NULL DEFAULT '' ");
        $stmt->execute();
    }
